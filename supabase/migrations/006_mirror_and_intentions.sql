...
CREATE POLICY "users own mirror sessions"
  ON mirror_sessions FOR ALL USING (auth.uid() = user_id);
...
CREATE POLICY "users own intentions"
  ON implementation_intentions FOR ALL USING (auth.uid() = user_id);
